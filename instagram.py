...
class InstagramBot():
    ...
    # метод проверяет xpath существует ли элемент на странице
    ...
    def get_all_followers(self, userpage):
        ...
        if self.xpath_exists(wrong_userpage):
            ...
            counts = browser.find_element_by_xpath("/html/body/div[1]/section/main/div/header/section/ul/li[2]/a/span").get_attribute('title')
            print(counts)
            time.sleep(4)
            counts = str(counts)
            counts = counts.replace(" ", "")
            followers_count = int(counts)
                                ...
                                if self.xpath_exists(
                                    ...
                                    try:
                                        if self.xpath_exists(
                                            ...
                                            follow_button = browser.find_element_by_xpath("/html/body/div[5]/div/div/div[2]/ul/div/li[3]/div/div[3]/button").click()
                                            print(f'Подписались на пользователя {page_owner}. Открытый аккаунт!')
                                    except Exception as ex:
                                        print(ex)
